...
class EmailService {
  ...
  async sendPasswordResetEmail(email, resetToken, userName) {
    try {
      ...
      const emailContent = {
        from: '"LeaveFlow" <mel.leaveflow@gmail.com>',
        to: email,
  ...
  async sendWelcomeEmail(email, userName) {
    try {
      ...
      const transporter = nodemailer.createTransport(this.config);
      await transporter.sendMail(emailContent);
      // sendWelcomeEmail fonksiyonu içinde, e-posta gönderiminden sonra
      console.log('Hoş geldin e-postası gönderildi:'); //, emailContent.to
      return true;
    } catch (error) {
      console.error('E-posta gönderimi hatası:', error);
      throw error;
